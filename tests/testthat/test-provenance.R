...
test_that("provenance is initialized on Survey creation", {
  s <- make_test_survey()
  expect_false(is.null(s$provenance))
  expect_s3_class(s$provenance, "metasurvey_provenance")
  expect_equal(s$provenance$source$initial_n, 10)
  expect_false(is.null(s$provenance$source$timestamp))
  expect_false(is.null(s$provenance$environment$metasurvey_version))
  expect_false(is.null(s$provenance$environment$r_version))
})
...
test_that("provenance records step history after bake", {
  ...
  s <- bake_steps(s)
  expect_length(s$provenance$steps, 2)
...
test_that("provenance tracks row count changes from filter", {
  ...
  expect_lt(prov_step$n_after, 100)
  expect_gt(prov_step$n_after, 0)
})
...
test_that("provenance records duration", {
  ...
  s <- step_compute(s, x2 = x * 2)
  s <- bake_steps(s)
  expect_false(is.null(s$provenance$steps[[1]]$duration_ms))
...
test_that("provenance_to_json returns valid JSON", {
  ...
  expect_false(is.null(parsed$source))
  expect_false(is.null(parsed$environment))
})
...
test_that("provenance_diff detects differences", {
  ...
  expect_false(is.null(diff_result$final_n))
})
...
test_that("provenance survives shallow_clone", {
  s <- make_test_survey()
  s2 <- s$shallow_clone()
  expect_false(is.null(s2$provenance))
...
test_that("workflow result has provenance attribute", {
  ...
  expect_false(is.null(prov$estimation$timestamp))
